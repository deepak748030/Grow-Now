    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center space-x-3">
          {selectedTopCategory ? (
            <span>
              {selectedTopCategory.title} ({selectedTopCategory.category.title})
            </span>
          ) : (
            <span className="text-gray-400">{placeholder}</span>
          )}
          ...
          {isLoading ? (
            ...
            <>
              ...
              {topCategories.map((topCategory) => (
                ...
                >
                  <span>
                    {topCategory.title} ({topCategory.category.title})
                  </span>
                </button>
              ))}
            </>
...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (watchedCategory) {
      const filteredTopCategories = allTopCategories.filter((topCat) => topCat.category._id === watchedCategory)

      setTopCategories(filteredTopCategories)

      // Reset top category and sub category if current selection is not valid

      const currentTopCategory = watch("topCategory")

      if (currentTopCategory && !filteredTopCategories.find((tc) => tc._id === currentTopCategory)) {
  ...
  }, [watchedCategory, allTopCategories, setValue, watch])

  // Filter sub categories based on selected top category

  useEffect(() => {
    if (watchedTopCategory) {
      const filteredSubCategories = allSubCategories.filter((subCat) => subCat.topCategory._id === watchedTopCategory)

      setSubCategories(filteredSubCategories)

      // Reset sub category if current selection is not valid

      const currentSubCategory = watch("subCategory")

      if (currentSubCategory && !filteredSubCategories.find((sc) => sc._id === currentSubCategory)) {
        setValue("subCategory", "")
      }
    } else {
      setSubCategories([])
  ...
  }, [watchedTopCategory, allSubCategories, setValue, watch])
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <TopCategoryDropdown
                    ...
                    onChange={(topCategoryId) => setValue("topCategory", topCategoryId)}
                    isLoading={isCategoriesLoading}
                  />
                ...
                <div>
                  ...
                  <SubCategoryDropdown
                    ...
                    onChange={(subCategoryId) => setValue("subCategory", subCategoryId)}
                    isLoading={isCategoriesLoading}
                  />

...
import { Trophy, Plus, Pencil, Trash2, X, Menu, Loader2, Search } from "lucide-react"
...
const getDailyTips = async () => {
    const response = await api.get<ApiResponse>("/goals")
...
const createDailyTip = async (formData: FormData) => {
    const response = await api.post<ApiResponse>("/goals", formData, {
...
const updateDailyTip = async (id: string, formData: FormData) => {
    const response = await api.patch<ApiResponse>(`/goals/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
    })
    return response.data.data[0]
}
...
const searchDailyTips = async (query: string) => {
    const response = await api.get<ApiResponse>("/goals/search", {
        params: { q: query },
    })
    return response.data.data
}
...
export default function GoalPage() {
    ...
    const onSubmit = async (data: DailyTipFormData) => {
        setIsLoading(true)
        try {
            // Check if we're at the 4 item limit when adding a new item
            if (!editingTip && dailyTips.length >= 4) {
    ...
    const handleEdit = async (tip: DailyTip) => {
        try {
            ...
            setValue("category", fullTip.category)
            ...
            <header className="sticky top-0 z-30 bg-gray-900 border-b border-gray-800 px-4 sm:px-6 py-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <h1 className="text-xl sm:text-2xl font-bold text-gray-100">Goals</h1>
                    </div>
                    <div className="flex items-center space-x-4">
                        <button
                            ...
                            disabled={dailyTips.length >= 4}
                            className="hidden sm:inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 border border-transparent rounded-md shadow-sm text-sm font-medium text-gray-900 bg-blue-400 hover:bg-blue-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                ...
                {showMobileMenu && (
                    <div className="sm:hidden mt-4 space-y-4">
                        <button
                            ...
                            disabled={dailyTips.length >= 4}
                            className="w-full flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-gray-900 bg-blue-400 hover:bg-blue-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <Plus className="w-4 h-4 mr-2" />
                            Add Goal
                        </button>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center space-x-4">
                                                ...
                                                <div>
                                                    ...
                                                    <span className="inline-block mt-1 px-2 py-0.5 text-xs rounded-full bg-gray-700 text-gray-300">
                                                        {tip.category}
                                                    </span>
                                                </div>
                                            </div>
                    ...
                    <div className="bg-gray-900 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto border border-gray-800">
                        <div className="sticky top-0 bg-gray-900 px-6 py-4 border-b border-gray-800 z-10">
                            <div className="flex justify-between items-center">
                                <h2 className="text-xl font-semibold text-gray-100">{editingTip ? "Edit Goal" : "Add New Goal"}</h2>
